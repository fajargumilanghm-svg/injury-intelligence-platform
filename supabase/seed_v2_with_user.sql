-- ============================================================================
-- SEED DATA v2 — With user_id support for RLS compliance
-- ============================================================================
-- 
-- INSTRUKSI PENTING:
-- 1. Buat user auth dulu (via /auth/register atau Supabase Dashboard)
-- 2. Dapatkan UUID user dari: Supabase Dashboard → Authentication → Users
-- 3. Ganti 'YOUR_AUTH_USER_UUID' di bawah dengan UUID tersebut
-- 4. Jalankan seluruh script ini di Supabase SQL Editor
--
-- ============================================================================

-- ─── Step 1: Setup Admin User (ganti UUID di sini!) ─────────────────────────

DO $$
DECLARE
  admin_uuid UUID := 'YOUR_AUTH_USER_UUID';  -- ← GANTI INI!
  admin_email TEXT := 'admin@injuryintel.com'; -- ← GANTI INI!
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM auth.users WHERE id = admin_uuid) THEN
    RAISE NOTICE 'User % found in auth.users', admin_uuid;
  ELSE
    RAISE WARNING 'User % NOT FOUND in auth.users!', admin_uuid;
    RAISE WARNING 'Please create user first via /auth/register or Supabase Dashboard';
    RAISE WARNING 'Then update admin_uuid above with the correct UUID';
  END IF;

  -- Insert/update user_profiles (agar role = administrator)
  INSERT INTO user_profiles (user_id, email, full_name, role, created_at, updated_at)
  VALUES (admin_uuid, admin_email, admin_name, 'administrator', NOW(), NOW())
  ON CONFLICT (user_id) DO UPDATE SET
    role = 'administrator',
    full_name = admin_name,
    updated_at = NOW();
  
  RAISE NOTICE 'User profile set to administrator for %', admin_email;
END $$;

-- ─── Step 2: Insert Athletes (dengan user_id untuk RLS) ──────────────────────

DO $$
DECLARE
  admin_uuid UUID := 'YOUR_AUTH_USER_UUID';  -- ← PASTIKAN SAMA DENGAN ATAS!
BEGIN
...
INSERT INTO athletes (user_id, full_name, age, gender, height, weight, sport, playing_position, dominant_side, training_experience, previous_injury_history) VALUES
  (admin_uuid, 'Alex Johnson', 24, 'male', 185, 82, 'Soccer', 'Forward', 'right', 'advanced', 'ACL reconstruction right knee (2022)'),
  (admin_uuid, 'Maria Garcia', 26, 'female', 170, 65, 'Basketball', 'Point Guard', 'right', 'advanced', 'Ankle sprain left (2023)'),
  (admin_uuid, 'James Smith', 22, 'male', 190, 88, 'American Football', 'Linebacker', 'left', 'intermediate', 'Concussion (2023)'),
  (admin_uuid, 'Sarah Williams', 23, 'female', 168, 60, 'Volleyball', 'Outside Hitter', 'right', 'intermediate', NULL),
  (admin_uuid, 'David Brown', 27, 'male', 178, 75, 'Rugby', 'Flanker', 'ambidextrous', 'advanced', 'Shoulder dislocation right (2022)'),
  (admin_uuid, 'Emma Davis', 21, 'female', 172, 63, 'Soccer', 'Midfielder', 'right', 'intermediate', 'Hamstring strain (2024)'),
  (admin_uuid, 'Michael Wilson', 25, 'male', 182, 80, 'Basketball', 'Small Forward', 'right', 'advanced', 'Stress fracture foot (2023)'),
  (admin_uuid, 'Lisa Anderson', 24, 'female', 175, 68, 'Soccer', 'Defender', 'left', 'intermediate', NULL),
  (admin_uuid, 'Chris Taylor', 28, 'male', 188, 85, 'American Football', 'Quarterback', 'right', 'advanced', 'ACL reconstruction left knee (2021)'),
  (admin_uuid, 'Anna Martinez', 22, 'female', 165, 58, 'CrossFit', 'Athlete', 'right', 'beginner', 'Lower back strain (2024)');
...
-- ─── Step 6: RTP Phases & Milestones (Alex Johnson) ───────────────────────

DO $$
DECLARE
  acl_injury_id UUID;
BEGIN
  SELECT id INTO acl_injury_id FROM injuries 
  WHERE injury_type = 'ACL Tear' 
  AND athlete_id = (SELECT id FROM athletes WHERE full_name = 'Alex Johnson' LIMIT 1) 
  LIMIT 1;
  
  INSERT INTO rtp_phases (injury_id, phase_number, phase_name, description, start_date, completion_date, status) VALUES
    (acl_injury_id, 1, 'Rest & Immobilization', 'Initial rest, protection, and pain management', '2024-01-15', '2024-02-15', 'completed'),
    (acl_injury_id, 2, 'Pain-Free Range of Motion', 'Restore full pain-free movement', '2024-02-16', '2024-04-01', 'completed'),
    (acl_injury_id, 3, 'Strength Recovery', 'Regain strength and neuromuscular control', '2024-04-02', '2024-06-15', 'completed'),
    (acl_injury_id, 4, 'Proprioception & Agility', 'Balance, coordination, and cutting movements', '2024-06-16', '2024-08-01', 'completed'),
    (acl_injury_id, 5, 'Sport-Specific Drills', 'Position-specific movements and skills', '2024-08-02', '2024-09-15', 'completed'),
    (acl_injury_id, 6, 'Full Training', 'Unrestricted team training and scrimmage', '2024-09-16', '2024-10-15', 'completed'),
    (acl_injury_id, 7, 'Competition Clearance', 'Medical clearance for match/game play', '2024-10-16', '2024-11-15', 'completed');

  INSERT INTO recovery_milestones (injury_id, milestone_date, milestone_type, description, completed) VALUES
    (acl_injury_id, '2024-02-15', 'medical', 'Splint/crutches discontinued', true),
    (acl_injury_id, '2024-04-01', 'medical', 'Full range of motion achieved', true),
    (acl_injury_id, '2024-06-15', 'strength', 'Quad strength 80% of contralateral', true),
    (acl_injury_id, '2024-08-01', 'proprioception', 'Single leg hop test passed', true),
    (acl_injury_id, '2024-09-15', 'sport_specific', 'Completed sport-specific drills', true),
    (acl_injury_id, '2024-10-15', 'full_training', 'Completed full team training', true),
    (acl_injury_id, '2024-11-15', 'rtp_clearance', 'Medical clearance granted', true);
END $$;

-- ─── Step 7: Physical Screenings ────────────────────────────────────────────
...
BEGIN
  ...
  RAISE NOTICE 'Training Entries: %', (SELECT COUNT(*) FROM training_entries);
  RAISE NOTICE 'Wellness Entries: %', (SELECT COUNT(*) FROM wellness_entries);
  RAISE NOTICE 'RTP Phases: %', (SELECT COUNT(*) FROM rtp_phases);
  RAISE NOTICE 'Milestones: %', (SELECT COUNT(*) FROM recovery_milestones);
  RAISE NOTICE 'Screenings: %', (SELECT COUNT(*) FROM physical_screenings);
  RAISE NOTICE '========================================';
  RAISE NOTICE 'You can now login and see all data!';
  RAISE NOTICE '========================================';
END $$;
